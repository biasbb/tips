...
- #### 23.合并k个有序链表
  ...
  - 把所有链表的节点值保存到vector中，排序vector然后再一个一个输出
    ...
    ```c++
    if(list.size()==0)
         return NULL;      //多了判断长短的一步，因为下面出现了list[list.size()-1],如果list为空的这个语句会造成越界操作
  ...
  - 使用优先队列
    ...
    > less<int>，less即<,即a<b<c   递增
    >
    > greater<int>  greater即>，即a>b>c  递减

    要构建小顶堆用`priority_queue<int, vector<int>, greater<int> > p`

    如果后面要比较的类型不是默认的数据类型，例如listnode->val，那么可以使用仿函数
    ...
    struct cmp
    {
        ...
        {
            return a->val > b->val;    //因为不是默认数据类型，因此要自定义比较函数，小顶堆要递减
        }
    };
  ...
  - 两种和优先队列思想一致但实现略有不同的方法
    ...
    - 使用multimap代替priority_queue，不用写比较函数

      > 可以重复的有序map
      >
      > 插入元素不影响原来的迭代器
      >
      > 若it为迭代器，it->first和it->second为map的pair中的元素引用
    ...
    - 使用类模板pair将int和ListNode*h合并成一个数据类型，即数据类型`pair<int,ListNode>`
...
- #### 155.最小栈
  ...
  public:
      ...
      void push(int x) 
      {
          if(data.empty())            //这儿注意一个栈空就行，不然栈空时minStack.top()是非法访问
          {
              data.push(x);
              minStack.push(x);
          }
...
- #### 452.用最少数量的箭射爆气球
  ...
  - 区间排序后比较维护的箭的位置（初始时就是第一个区间的有边界）与当前区间左边界间的关系，if当前区间的start在shoot之后，那么就加一根箭
...
组合和组合Ⅱ的区别1是集合中的元素能否重复，能的话回溯的时候从索引0开始；区别2也是nums是否重复然后去重的那个操作
...
- #### 90.子集Ⅱ
  ...
  public:
      ...
      {  
          ...
          for(int i=start;i<nums.size();i++)
          {
              if(i>start&&nums[i]==nums[i-1]) //把递归树同一层相等的元素去掉
                  continue;
              temp.push_back(nums[i]);
  ...
  public:
      void backtrack(int start,int target)
      {
          ...
          for(int i = start; i < candidates.size()&&target>=candidates[i]; i++)
          {
              ...
              backtrack(i,target-candidates[i]);  //还是从i开始，因此每个数可以重复，下一次递归还是从当前数字的索引开始
              path.pop_back();
          }
      }
...
- #### 51.N皇后
  ...
  public:
      ...
      {
          int n=board.size();  //第n行的索引是n-1，填完之后进入下一次递归，此时row的索引是n，此时才退出回溯，因此回溯中止条件为n=board.size()，if是board.size()-1的话，当属第二行填完就退出了
          if(row==n)
              res.push_back(board);
...
- #### 315.计算右侧小于当前元素的个数
  ...
  3. 利用归并排序的性质，归并排序二分完之后，在归并有序数组的过程会记录逆序数
     ...
     {
         public:
         void merge(vector<pair<int,int>> &a,int low,int mid,int high,vector<int> &count)
         {
             int i=low,j=mid+1,k=0;  //low为左区的第一个元素，mid+1为右区的第一个元素。左区的最后一个元素为mid，右区的最后一个元素是high
             vector<pair<int,int>> temp(high-low+1);   //用临时数组存储
     
             while(i<=mid&&j<=high)   //小的元素先存，类似于合并有序链表
             {
                 if(a[i].first<=a[j].first)
                 {
                     count[a[i].second] += j - (mid + 1);
                     temp[k++] = a[i++];
                 }
                 else
                     temp[k++]=a[j++];
         ...
         void MergeSort(vector<pair<int,int>> &num,int low,int high,vector<int> &count)
         {
             if(low>=high)
                 return;
             int mid=low +(high-low)/2;
     
             MergeSort(num,low,mid,count);     //递归排序左半边
             MergeSort(num,mid+1,high,count);  //递归排序右半边
             merge(num,low,mid,high,count);    //对排序好的两边进行合并
         }
         vector<int> countSmaller(vector<int>& nums)
         {
             ...
             MergeSort(num,0,num.size()-1,count);
             return count;
         }
     };
     ```
...
- #### 207.课程表
  ...
  要利用图的广度优先搜索中的拓扑排序，其实题目本质上就是判断图有没有环，利用队列实现广度优先搜索
  ...
  public:
      bool canFinish(int numCourses, vector<vector<int>>& prerequisites) 
      {
          ...
          while(!q.empty())
          {
              ...
              for(auto x:adjacencyList[p])   //队头出队以后，将它邻居表中对应节点的入度减一，然后看入度是否为0，是的话进队
              {
...
- #### 35.搜索插入位置
  ...
  用weiwei大佬的模板，两种写法。**第一种是从左边逼近，第二种是从右边逼近**，因此在下面的34题中，第一种可以用来找第一个出现的位置，而第二种找最后一次出现的位置
