- #### 1.两数之和
  ...
  要注意数组中有相同元素的情况，相同的key后面的value会覆盖前面的，所有比的时候只能比当前索引不能比value
  ...
  - 既然结果一定存在，那么也可以排序后从两头向中间逼近，但是排序之前要先给每个元素一个索引，用pair
...
- #### 9.回文数
  ...
  while(x > ans)。跳出循环后如果是原数有偶数个数字，那么x==ans就表明是回文数；如果有奇数个数字，那么跳出循环后x==ans/10就表明是回文数。
...
- #### 14.最长公共前缀
  ...
  只比较字典序的第一个和最后一个，他们的公共前缀即为所有元素的公共前缀
  ...
  ```c++
     vector<vector<int>> threeSum(vector<int>& nums) 
    {
          ...
          for(int i=0;i<n-2;i++)
          {
              if(nums[i]>0)
                  break;
              if(i>0&&nums[i]==nums[i-1]) //不能和前面的相等，因为前面的已经遍历过了，但是可以和后面的相等，类似于回溯那儿的去重，i>0限制在同一层，同一层内i不能和它前面的相等
                  continue;
              int j=i+1;
              int k=n-1;
              while(j<k)
              {
                  ...
                  else
                  {
                      ...
                      j++;
                      while(j<k && nums[j]==nums[j-1]) //不能和前面的重复，和i一样
                          j++;
                      k--;
  ...
  public:
      ListNode* mergeKLists(vector<ListNode*>& lists) 
      {
          ...
          while(!m.empty())
          {
              ...
              p=p->next;
              if(it->second->next)
                  m.insert({it->second->next->val,it->second->next});
              m.erase(it);
          }
          return dummy->next;
      }
...
- #### 47.全排列Ⅱ 会重复
  ...
  public:
      ...
      void backTrack(vector<vector<int>> &res,vector<int> &path,vector<bool> &used,vector<int> &nums)
      {
          ...
          for(int i=0;i<nums.size();i++)
          {
              if(used[i] || i>0&& nums[i]==nums[i-1]&&!used[i-1])   //nums[i]==nums[i-1]同时used[i-1]==0这时才可以剪枝，即和他相同同时i-1没有被用即不在已便利的序列里，不在已便利的序列证明前面和它相同的i已经遍历所有序列且撤销选择，再用相同的i再遍历一次就重复了，所以要确定used[i-1]==0
                  continue;
...
- #### 113.路经总和Ⅱ
  ...
  public:
      ...
      void backtrack(TreeNode *root,vector<vector<int>> &res,vector<int> &temp,int sum)
      {
          ...
          temp.push_back(root->val);  //先push之后再判断，不然判断之后push进去的temp没有当前结点的val
          if(root->val==sum&&root->left==NULL&&root->right==NULL)
          {
               res.push_back(temp);
          }
  ...
  - 常规的使用深度优先搜索，记录每次搜寻一个节点并保存他的路径，然后从根节点开始比较两个节点的路径
    ...
    public:
        ...
        void dfs(vector<vector<TreeNode*>> &res,vector<TreeNode*> &path,TreeNode *root,TreeNode *p)
        {
            ...
            path.push_back(root);
            if(root==p)
                res.push_back(path); //满足条件push就行了，不要return也不要clear（）,因为上层的还要pop（）
  ...
  1. ```c++
     ...
     public:
         int searchInsert(vector<int>& nums, int target) 
         {
             ...
             while(begin<end)
             {
                 mid=begin+(end-begin)/2;  //这种下取整mid可以取到左边界，但是永远娶不到右边界，因此下面用begin=mid+1来保证可以取到有边界。mid被分配搭配左边部分
                 if(nums[mid]<target)   
                     begin=mid+1;
                 else
                     end=mid;
  ...
  2. ```c++
     ...
     public:
         int searchInsert(vector<int>& nums, int target) 
         {
             ...
             while(begin<end)
             {
                 mid=begin+(end-begin+1)/2;  //+1的话mid永远访问不到左边界，但是可以访问到右边界因此下面用end=mid-1来取左边界。mid被分配到右边部分
                 if(nums[mid]>target)   
                     end=mid-1;
                 else
                     begin=mid;
          ...
          while(l<r)
          {
              ...
              if(nums[mid]<nums[r])
              {
                  if(nums[mid]<=target && target<=nums[r])
                      ...
                      r=mid-1;   //搜索区间为[l,mid-1]。出现mid-1，所以上面要+1再除以2
              }
              else
              {
    ...
    bool wordPattern(string pattern, string str) 
      {
        vector<string> res;
          ...
          if(res.size() != pattern.size())   //长度都不一样，那么直接淘汰
              return false;
  
          unordered_map<char,string> m;                //保证pattern中相同的字母对应的单词相同
          for(int i=0;i<pattern.size();i++)
          {
              if(m[pattern[i]]=="")
                  m[pattern[i]]=res[i];
              else if(m[pattern[i]]!="" && m[pattern[i]]!=res[i])
                  return false;
          }
          //但是并没有保证不同的字母对应的单词不同，可以用set来去重，保证不同的字母对应的单词不同
          set<char> a(pattern.begin(),pattern.end());
          set<string> b(res.begin(),res.end());
      ...
      int largestRectangleArea(vector<int>& heights) 
      {
          ...
          for(int i=0;i<heights.size();i++)
          {
              while(!s.empty() && heights[i]<heights[s.top()])
              {
                  ...
                  ans=max(ans,(right-left+1)*heights[height_index]);   //两个索引构成的闭区间的长度即为r-l+1，再乘以高度即为以heights[height_index]为高的矩形的最大面积
              }
              s.push(i);
          }        
          return ans;
