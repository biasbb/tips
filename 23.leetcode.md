...
- #### 33.搜索旋转数组
  ...
  public:
      int search(vector<int>& nums, int target) {
          ...
          int l = 0, r = n - 1;
          while (l <= r)   //这儿用等号就是连最后l=r也判断了，这样退出循环时如果没有返回mid那么必然找不到，就返回-1；上面用<的话最后l=r的时候没有在循环里判断因为我们上面找的索引而且找的数可能不在这个数组里，因此最后返回left的时候如果找到，就是这个索引，如果找不到就是插入的位置；如果是判断找的数在不在数组里，那么循环就用l<=r，如果找到的是索引就用l<r
          {
              int mid = (l + r) / 2;
              if (nums[mid] == target) 
                  return mid;
              if (nums[l] <= nums[mid]) //看上面的图，l<=mid说明l到mid是递增序列.而且这儿的小于等于是因为上面用的是整数除，会把小数去掉，因此当l=0,r=1时会出现l==(l+r)/2，所以要用<=
              {
