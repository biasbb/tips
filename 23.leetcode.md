...
- #### 47.全排列Ⅱ 会重复
  ...
  public:
      ...
      void backTrack(vector<vector<int>> &res,vector<int> &path,vector<bool> &used,vector<int> &nums)
      {
          ...
          for(int i=0;i<nums.size();i++)
          {
              if(used[i] || i>0&& nums[i]==nums[i-1]&&!used[i-1])   //nums[i]==nums[i-1]同时used[i-1]==0这时才可以剪枝，即和他相面的相同同时i-1没有被用即不在已便利的序列里
                  continue;
...
- #### 409.最长回文串
  ...
  回文串要求两边对称，如果字符出现偶数次，那么就一边方一半，满足要求；如果是奇数次的话，就取i-1个加入结果。

  最后的中间可以再放一个，那么如果有奇数元素，就结果加一，如果没有奇数元素，就不用加了。
...
- #### 290.单词规律
  ...
  public:
      bool wordPattern(string pattern, string str) 
      {
          ...
          string hash[128];  //哈希表，保证相同的key对应的value相同
          ...
          set<char> s;
          set<string>b;     //使用set保证不同的key对应的value不同
          for(int i=0;i<res.size();i++)
          {
              s.insert(pattern[i]);
              b.insert(res[i]);
...
- #### 473.火柴拼正方形
  ...
  public:
      ...
      {
          if(n==nums.size())
              return res[0]==res[1]&&res[1]==res[2]&&res[2]==res[3]&&res[0]==target?1:0;
