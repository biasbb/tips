...
- ### 建表约束条件
  ...
  1. **主键约束**（一张表只能有一个主键，但是这个主键可以有多个字段值即联合主键，这也就是联合主键要一起add的原因）
  ...
  2. **唯一约束**（多少个都行）
     ...
     约束修饰的字段的值不能重复，可以同时添加多个，多个话也是多个键在一起不重复就行
  ...
  5. **外键约束**
     ...
     **即把class_id关联到classes的id字段，这样class_id插入时的值只能为classes的id字段值的其中一个**
     ...
     1. 主表中被关联的键必须是主键
     2. 要删除外键
        - 使用show create table student找到约束的名字
...
- ### 查询练习
  ...
  14. 查询所有学生的sname，cnumber和degree列。注意这些来自不同的表。多表查询
      ...
      ```sql
      select sname,cnumber,degree from score,student where score.snumber=studeng.snumber;
      ```
  ...
  16. 查询95031班学生每门课的平均分
      ...
      select cno,avg(degree) from score where snumber in(select snumber from student where class='95031') group by cnumber;
      ```
...
- #### sql的四种连接查询
  ...
  2. 外连接，有左外连接和右外连接。
     ...
     0就是内连接，12分别是左连接和右连接，而3本应是全连接，但是mysql没有全连接，要实现的话用左连接union右链接来实现
...
- ### 事务的四大特征
  ...
  3. repeatable read（这时mysql默认的隔离级别）
     ...
     解决了上面不可重复读的现象，对同一笔数据的读取结果是相同的，不管其他事务是否在对共享数据进行更新。
