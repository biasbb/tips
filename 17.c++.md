...
- #### find和find_if
   ...
   vector<int>::iterator it=find_if(a.begin(),a.end(),select);
   ```

- #### 迭代器相减

   vector的迭代器相减结果是两个迭代器中间的元素个数，左开右闭区间。

   ```c++
   vector<int> a{1,2,3,4}
   int b=a.end()-a.begin();   //b的值为4
   ```
...
- #### 空类
   ...
   空类的大小是1而不是0，起到一个占位的作用。如果是0的话声明一个对象数组class A[10]，每一个对象占用的空间都是0，就无法区分每个元素了
...
- #### return不可返回指向栈内存的指针
   ...
   int *p()
   {
       int a=1;
       return &a;
   }                     //返回的指针指向a，而a是局部变量，存储在栈上，栈内存在函数调用结束时会自动销毁，因此返回的p()里的内容是位置的，所以下面的结果无输出
   int main()
   {
      int *b=p();
      cout<<*b;     
...
- #### this指针
   ...
   类的非静态成员函数都有一个隐含的参数this指针，指向当前对象。同时成员函数中所有的类的成员均被隐含地施加this指针，即缺省的情况下类的非静态成员函数中所使用的非静态成员是this指针所指向的对象成员。
