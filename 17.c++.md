...
1. 局部变量（函数内部）
   ...
   - 普通局部变量。即为在函数内部定义的变量，**初始化时未赋值则自动初始化为0**。存储于进程栈空间，使用完立刻释放内存。
...
4. 类的静态数据成员
   ...
   - **如果不初始化就调用的话是非法的，编译出错，普通的成员变量或者普通的局部或者全局静态变量不初始化的话默认就是0，静态成员函数这点比较特殊。**
   ...
   - 访问的话类名和对象名都可以，但是普通数据成员只能通过对象。静态数据成员可以使用A::value来访问，即直接使用类名访问
...
- ### 不同文件之间读取
  ...
  1. 不同文件的非静态全局变量读取的话，直接加个extern然后声明一下就行。
  ...
  2. c++源文件A.cpp想读取另一个源文件B.cpp的函数int add(int x)
...
- ### 友元
  ...
  首先是友元的概念，友元有三种方式。**（第二种友元成员函数比较特殊，mark一下）**
...
- ### public，private和protected
  ...
  - 访问控制修饰符
    ...
    - protected：对本类的成员函数以及本类的派生类和友元可见，即派生类和友元类的成员函数可以访问它；对其他不可见，包括类对象，类对象想访问保护成员只能先用类的public成员函数访问保护成员，然后类对象再访问public成员函数。

    - private：对本类的成员函数及友元（三种方式）可以访问（**友元类的成员函数访问其实是通过对象来访问，只不过这个对象在类的成员函数里而不在类外**），子类不可见
...
- ### 位域
  ...
  > 只能在类外初始化
  >
  > 如果用超过指定位数的数去赋值，那么只会保存指定位数的数字。例如a::2，即a占用2bit，如果b.a=2，那么a就等于2，即0B10，如果b.a=4，那么a即位0，因为4是100，而a只能保存2位，因为a是0B00
  ...
  struct A
  {
      ...
      unsigned int b:4;
                ...
                //size(i)是4,a和b的位数之和没有超过一个int的32位；一旦他们的位数之和超过了32位，那就要再用一个int，size(i)即为8
  ```

- ### 宏定义和内联函数

  **inline必须加在函数定义的前面才能生效，加在函数声明前面是没有效果的**

  - 相同点
  1. 内联函数和宏函数一样都是在调用处把代码展开，省去了参数压栈、栈帧的开辟和回收等的函数调用的过程，从而提高了程序的运行
  - 不同点
    1. 宏定义在预处理阶段，而inline是编译阶段。因此后者可以安全检查
    2. **类的成员函数如果定义在类中，那么默认就是内联函数；如果定义在类外，那就是普通函数**
  - inline的缺点
    1. 代码膨胀，如果函数体本身执行时间很长，比函数调用的开销还大，那inline就没有意义了，反而会消耗更多的内存
    2. 是否内联，并不是程序员可以控制的。我们加inline关键字，只是对编译器的建议，**实际编译的时候是否对函数内联，决定权在编译器**，编译器有自己的优化规则
...
- ### 函数指针和指针函数
  ...
  using P=int(*)(int,int);   //这儿的括号不能去掉，表示是函数指针类型
...
- #### malloc/free和new/delete区别（new既是操作符也是关键字）
  ...
  1. new/new[]：先调用底层的malloc分配内存，然后调用构造函数创建对象
  2. delete/delete[]：先调用析构函数清理对象资源，再调用底层的free释放空间
...
- #### static_cast,dynamic_cast,reinterpreter_cast和const_cast
   ...
   > 转换对象指针上下行都可以，但是对象的话只能上行转换，即从子类变成父类。
   >
   > A a=B();这种是上行转换，相当于把一个B类对象变成了A类对象
   >
   > 但是下行转换会失败，B b=A()；即从父类到子类，这种是错误的
   ...
   - dynamic_cast
     ...
     这种属于安全的下行转换，使用static也可以。但是如果把21行改成Base *a=new Base();此时a指向父对象，下面的转换就是不安全的，因为b由a转换而来而a指向父对象，根本没有erzi这个数据成员，b->erzi这个操作就是不安全的。这时使用dynamic会动态检查出来这个不安全，而且会把指针b置为0；但是用static的话就不会发现这个不安全，最后23行的结果为father，24行直接越界访问，一般会赋值为0。不安全。因此下行转换用dynamic更安全
...
- ### 内存泄漏与智能指针（智能指针需要使用memory头文件）
   ...
   - shared_ptr
     ...
     指向同一个对象的share_ptr公用一个引用计数，它记录了有多少个shared_ptr指向该对象
...
- #### const
   ...
   1. 修饰变量表示该变量不可以被改变
   2. 修饰指针，分为指针常量和常量指针
   3. 修饰引用，指向常量的引用（即引用常量，引用的对象是一个常量，但是注意没有常量引用，引用引用本身就是不可变的）
   4. 修饰成员函数，表示该成员函数不能修改成员变量。
   ...
   - const修饰变量
     ...
     const在*右边，指向的变量为const，即数据不能改，指向能改

     const在*左边，指针为const，即数据能改，指向不能改
...
- ### 文件读写
   ...
   **如果事先没有这个文件新建的时候一定要加上ios::out,而且如果有了这个文件再次写的时候不加ios:：out的话是半覆盖写而不是全覆盖，加了才是全覆盖**
   ...
   //写文件操作，创建文件流对象之后用outfile<<往里写就行了，\n是换行符，这里使用绝对路径因为clion默认的路径并不在untitled下而是在untitled下的cmake-bulid-debug下
       ...
       getline(infile, s,'\0');  
       //getline函数原型为	istream& getline (istream& is, string& str, char delim);分隔符默认为\n,可以手动指定，指定的话就是读到分隔符就停，然后再getline的话就从上次读到的地方继续读
       直接infile>>s也可以，和cin一样碰到一个特殊符号就断一下
...
- #### 复制构造函数
   ...
   - 复制构造函数调用的条件
     ...
     3. 函数的返回值为类对象
        ...
        这种情况下可能会引发编译器的copy elision（省略），从而将调用复制构造函数的过程省去，
...
- #### 让函数在main()之前/之后执行
   ...
   //main之后的话就改成destructor和after，当然这儿的函数名无所谓，用before和after是便于理解
   ```
...
- #### 去除字符串中的空格
   ...
   使用find函数，find函数的形参为find('a')，a第一次出现位置的索引，如果没找到返回string::npos(这是一个固定值的整数），类似于set的find，没找到返回set.end()
   ...
   while(index!=string::npos)
   {
       ...
       if(index!=string::npos)
           a.erase(index,1);  //string的erase函数如果给迭代器就删除迭代器位置的元素，如果给pos的话默认把从pos开始的全删掉
   }
   ```
...
- #### 函数重载小问题
   ...
   还有就是**重载必须是参数的个数或类型不一样**，如果只是修改函数的返回类型，并不是重载，是无法通过编译的
   ...
   double f(int a,int b)   //这样并不能看作重载，存在函数重定义，是错误的
   {
       return a+b;
   }
   ```
...
- #### 时钟函数
   ...
   `time_t  a=time(NULL);`time_t是long long类型
...
- #### void*，空指针和野指针
   ...
   - 空指针是赋值为0的指针，它的地址即为0，指向内存区间的最下面。但是它是没有指向的，对一个空指针取值属于非法操作
...
- #### memset函数和小端序(头文件string.h里)
   ...
   **函数解释：将s中当前位置后面的n个字节 用 ch 替换并返回 s 。**
   ...
   即把a中的每个字节都变成1，这儿的第二个形参只取最低位的8位，1的话就是0x01，然后把a中元素的每个字节都替换成第二个参数
   ```
...
- #### typedef
   ...
   > typedef不能给模板取别名，而using可以，其他的用法二者一样，鼓励用using代替typedef
...
- #### typename（只能用在结构体或者类或者模板函数）类型解释，明确告诉编译器我后面跟的是一个类型的名字而不是其他的数据成员或者静态变量名啊什么的
   ...
   public:
       ...
       typename T::a t;  //加了typename表示T::a是个类型名
       int b2=t.r;      
   };
...
- #### vector的全排列函数
   ...
   next_permutation(a.begin(),a.end()) 返回a的下一个排列排列，**它的返回值是bool类型**。这个全排列的次序是按照字典序的，如果下一个没有就返回空。所以如果要获得全部的全排序的话使用前必须把数组排序，然后再用这个函数，不然它只会返回当前排序的下一个字典排序
   例如1234，下一个字典序就是1243
...
- #### avl（严格平衡的二叉搜索树）和rb_tree

   1. avl是严格平衡，而rb-tree是弱平衡
   2. avl由于严格平衡，所以插入删除结点时rebalance的频率更高。因此对于插入删除操作，rb的效率更高；但是由于严格平衡，所以avl的查找效率更高
   3. 二者的查询复杂度都是logn
...
- #### STL的list的splice函数（[splaɪs],剪接的意思）

   三种重载形式

   void splice ( iterator position, list<T,Allocator>& x );

   > a.splice(a.begin(),b)：把b的所有元素都剪过来接到a.begin()处，左闭区间

   void splice ( iterator position, list<T,Allocator>& x, iterator it );

   > a.splice(a.begin(),b,b,begin())：只剪开头
...
- #### 强制类型转换
   ...
   int(a)和(int)a这两种效果一样

   前者是c++的语法，类似于构造函数创建对象；
...
- #### STL的交集，并集和查集函数
   ...
   1. 使用前必须保证原容器有序
   2. 使用back_inserter的话目标容器不用初始化大小，但是如果用c.begin()的话，c提前必须分配好合适的大小
   3. difference求的是a有而b没有的，而symmetric_difference求的是两个集合不重复的
