...
1. 局部变量（函数内部）
   ...
   - 普通局部变量。即为在函数内部定义的变量，编译器不会对其进行初始化。存储于进程栈空间，使用完立刻释放内存。未赋初值将会初始化为1
...
- ### string，(const) char*和char[]
   ...
   `const char*`指向的是常量字符串，即指向的内容不能修改，但是指向可以修改，即可以指向其他字符串
...
- ### 多个if与多个else if
   ...
   前者的else之和最接近的if匹配，他两势必有一个会执行，而前面的if只要满足了也会执行
...
- ### 使用数组作为函数参数
   ...
   - 普通的一维和二维数组
     ...
     这三种函数定义方式是一样的，编译器会自动将数组名转换成指向数组第一个元素的指针。在函数里操作数组元素的时候可以直接a[2]这样访问，但是返回sizeof(a)值是8，因为在64位系统指针的值是8字节。而且因为用的是指针，所以函数里修改之后原数组值也还修改
   ...
   - ### 杨辉三角
     ...
     void yangHuiTriangle(int n,int **a)
     {
         for(int i=0;i<n;i++)
             a[i][0]=a[i][i]=1;
         for(int i=2;i<n;i++)
...
- #### static_cast,dynamic_cast,reinterpreter_cast和const_cast
   ...
   - dynamic_cast
     ...
     {
         CBase *b;
         CDerived *a=dynamic_cast<CDerived *>(b);
     }
     ```

     而且dynamic可以识别出不安全的下行转换，但并不抛出异常，而是将指针置为NULL
...
- ### 内存泄漏与动态指针
   ...
   - auto_ptr
     ...
     c98定义的，c++11之后 舍弃改为使用unique_str，因为auto_str会造成潜在的内存泄漏
   ...
   - shared_ptr
     ...
     shared_ptr <int> q=p;   //这三种方式都可以得到shared_ptr
     ```

     p.use_count()就是引用次数，1行之后p.use_count()是1，3之后变为2.

     p.get()获取原指针，egint *a=p.get()，即可以把智能指针给普通指针

     p.reset()，如果括号里是空的，那么引用计数减一；如果是reset(new int(xxx))，那么首先生成新对象，然后将新对象交给p保管，原来的析构掉

   - weak_ptr

     为了解决shared_ptr循环引用的问题,不会改变引用计数的值
...
- #### const
   ...
   - const修饰变量
     ...
     const int *c;        //c为常量指针，为指向常整数的指针，所指向的内存数据不能修改，但是指针的指向可以修改
...
- #### 获取当前路径
   ...
   getcwd会把当前工作目录的绝对路径复制到复制到buf所指的内存空间中，size_t_size是buf的大小，普通用法是
   ...
   但是如果路径长度大于size，会返回NULL，错误。但如果定义path时足够大，又会造成内存浪费，因此我们可以令buf为NULL并使size为0来使getcwd调用new动态地给buf分配内存,最后记得delete path即可。
...
- #### 隐式类型转换
   ...
   - 可以用单个形参来调用的构造函数定义了从形参类型到该类类型的一个隐式转换。可以用单个形参来调用构造函数，并不是说构造函数只有一个形参，也可以有多个形参，只是有多个形参的话那么形参都是有默认实参的
     ...
     int main() 
     {
         ...
         cout << a.isSameName(string("bob")); //隐式类型转换，string->A。如果不想让程序使用默认的隐式类型转换，在构造函数前面加explicit关键字
...
- ### 赋值运算符
   ...
   需要显示的提供赋值运算符重载函数的两种情况

   **第一种不是必须用，也可以不用，但是如果类的成员变量里有指针类型，那么必须声明赋值运算符函数**

   - 用非类A类型的值给类A对象赋值时（**当然这种情况下可以不提供赋值运算符重载函数，而只提供相应的构造函数**）
   ...
   - **当用类A类型的值为类A的对象赋值，且类A的数据成员中含有指针的情况下，必须显式提供赋值运算符重载函数**
     ...
     > 总之如果类的成员变量里有指针，要使用对象赋值必须显示的声明一个以常量类引用为参数的赋值运算符重载函数。
     >
     > 如果没有指针，那就不用显示声明这样的函数，编译器会自动创建一个，对象赋值直接赋就行了。如果A只有一个成员变量且想用非A类型的值直接给A赋值的话可以定义一个赋值函数或者构造函数，且赋值函数的调用顺序在构造函数之前
